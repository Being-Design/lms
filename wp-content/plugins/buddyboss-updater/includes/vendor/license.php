...
class BBoss_License_Helper {
    ...
    protected   $_product_keys       = array( ),
                ...
                $_api_host          = 'https://www.buddyboss.com/',
                ...
                //these should be false, unless api site requires http authentication.
                //@todo set these to false( or update values if required ).
    ...
    public function refetch_license_status( $license_details ){
        ...
        $request_url = add_query_arg( $request_params, $this->_api_host );
    ...
    public function refetch_licenses( $licenses ){
        ...
        $rargs = array( 'timeout' => 50, 'body' => array( 'licenses' => $licenses ) );
        ...
        $q_response = wp_remote_post( $request_url, $rargs );
    ...
    public function activate( $key, $email ){
        if( empty( $key ) || empty( $email ) )
            return array( 'status'=>false, 'message' => __( 'Error - Invalid values!', 'buddyboss-updater' ) );
        ...
        if( is_wp_error($q_response) || ($q_response['response']['code'] != 200) )
            return array( 'status'=>false, 'message' => __( 'Error - Service unavailable!', 'buddyboss-updater' ) );
