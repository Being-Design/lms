...
if ( !class_exists( 'BuddyBoss_Updater_Admin' ) ):
    ...
    class BuddyBoss_Updater_Admin {
        ...
        public function setup () {
            ...
            add_action( 'wp_ajax_updater_bb_connect_received_message', array( $this, 'updater_bb_connect_received_message' ) );
        }
        ...
        public function admin_menu () {
            $this->page_hook_suffix = add_submenu_page(
                $this->settings_page, __( 'Welcome to BuddyBoss Updater', 'buddyboss-updater' ), __( 'Licenses', 'buddyboss-updater' ), $this->capability, $this->plugin_slug, array ( $this, 'options_page' )
            );
        ...
        public function print_settings_tabs () {
            ...
            foreach ( $this->packages as $package ) {
                ...
                switch ( $package_status ) {
                    case 'active':
                        $dashicon_class = 'admin-plugins yes';
                        break;
                    case 'inactive':
                        $dashicon_class = 'admin-plugins';
                        break;
                    case 'active_indirect':
                        $dashicon_class = 'admin-plugins yes indirect';
                        break;
                }
        ...
        public function admin_notice () {
            ...
            //check if we should show any notice at all
            if( bblicenses_switch__show_admin_notices() != 'yes' ){
                return;
            }
            ...
            if( !empty( $notices ) ){
                ...
                echo "<div class='notice notice-warning'>" . $notice
                . "<p><button class='button button-small' onClick='BBossULDissmissNotice(this);'>" . __( "Don't remind me", "TEXTDOMAIN" ) . "</button></p>"
                . "</div>";
                ?>
            ...
            if ( !$has_inactive_packages )
                return;

            $settings_link = "<a href='" . admin_url( 'admin.php?page=' . $this->plugin_slug ) . "'>" . __( 'activate your product licenses', 'buddyboss-updater' ) . "</a>";
        ...
        public function updater_bb_connect_received_message(){
            ...
            $retval = array( 'status' => true, 'message' => 'Done', 'redirect_to' => $this->plugin_settings_url  );
            $retval['message'] = __( 'Your account was successfully connected. No new activations have been made. If you are attempting to activate a new license key, please make sure that the product is installed on your site, and that an active license key is available in your BuddyBoss.com account.', 'TEXTDOMAIN' );
        ...
        public function process_bulk( $licenses ){
            ...
            if( !empty( $licenses_updated ) ){
                $retval['message'] = sprintf( __( 'Congratulations! License keys for the following product(s) have been activated: %s', 'TEXTDOMAIN' ), implode( ', ', $licenses_updated ) );
            } else {
                //no licenses updated
                //it can be becuase :
                ...
                //1. $licenses is empty
                if( empty( $licenses ) ){
                    $retval['message'] = __( 'The connection was successful, however, no license key was activated. We could not find any active license in your account, for any of the installed BuddyBoss products.', 'TEXTDOMAIN' );
                }
