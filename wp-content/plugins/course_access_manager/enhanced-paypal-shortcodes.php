...
define('LDP_TWPW_NAME', 'Enhanced Paypal Shortcodes');	// Name of the Plugin
define('LDP_TWPW_VERSION', '0.5a');			// Current version of the Plugin
define("LDP_ALT_ADD","Add to cart (Paypal)");	// alternate text for "Add to cart" image
define("LDP_ALT_VIEW","View Paypal cart");		// alternate text for "View cart" image
define("LDP_ALT_SUBS", "Subscribe Now (Paypal)");   // alternate text for "Subscribe" image
...
if ( !function_exists('learndash_plus_enhanced_paypal_shortcode') ) {
function learndash_plus_enhanced_paypal_shortcode($atts) {
$atts = shortcode_atts(
	Array(
		'type'			=> '',
		'textalign'		=> '',
		...
		'rm' 			=> '2',
		'lc' 			=> '',
		'cbt' 			=> __('Complete Your Purchase', 'learndash-plus'),
		'cn' 			=> '',
		'pagestyle' 	=> 'paypal',
		'notifyurl' 	=> '',
		'notifyurl2'	=> '',
...
switch($atts['type']):
	...
	$code = '
		...
		// Define Image to Use	
        if ($atts['imageurl']) { 
                $code.='<input type="image" src="'.$atts['imageurl'].'" border="0" name="submit" alt="'.LDP_ALT_ADD.'"';
                if ($atts['imagewidth']){
                         $code.=' width="'.$atts['imagewidth'].'"';
                 }
                $code.=' class="ppalbtn">';
        } else {
                $code.='<input type="image" src="' . $buttonUrl . '" border="0" name="submit" alt="'.LDP_ALT_ADD.'" class="ppalbtn">';
        }
		...
		if ($atts['custom']) {
			$code.='<input type="hidden" name="custom" value="'.$user_id.'">';
        }	
       if ($atts['scriptcode']) { 
...
case "subscribe":
	$code = '
        ...
        if ($atts['imageurl']) { 
            ...
            $code.='<input type="image" src="https://www.paypal.com/en_AU/i/btn/btn_subscribeCC_LG.gif" border="0" name="submit" alt="' . __('PayPal - The safer, easier way to pay online.', 'learndash-plus') . '" class="ppalbtn">';
		}
...
$code = '<div style="';        
	...
	$code.='<input type="hidden" name="business" value="'.$atts[email].'">
        ...
        if ($atts['imageurl']) { 
                ...
                $code.='<input type="image" src="https://www.paypalobjects.com/en_AU/i/btn/btn_cart_LG.gif" border="0" name="submit" alt="'.LDP_ALT_ADD.'" class="ppalbtn">';
        }
...
endswitch;
return apply_filters('learndash_plus_payment_button', array('code' => $code, 'atts' => $atts)) ;
}
}

add_shortcode('ldp_paypal', 'learndash_plus_enhanced_paypal_shortcode');
add_filter('learndash_plus_payment_button', 'learndash_plus_payment_button_free',100);
function learndash_plus_payment_button_free($button) {
	if(empty($button) || ($button['atts']['type'] == 'paynow' && ($button['atts']['amount'] == "0.00")) || ($button['atts']['type'] == 'subscribe' && ($button['atts']['a3']  == "0.00" )))
	{
		$user_id = get_current_user_id();
		
		if(!empty($user_id))
		{
			$levelid = $button['atts']['itemno'];
			$current_level = intVal(get_user_meta($user_id, 'learndash_plus_level', true ));
			if($current_level == $levelid)
			return "";
			
			if(!empty($_POST['learndash_plus_free_signupsubmit']) && !empty($_POST['learndash_plus_membership_id']) && !empty($_POST['learndash_plus_membership_id']) && $levelid == $_POST['learndash_plus_membership_id'])
			{
				$levelid = $_POST['learndash_plus_membership_id'];
				$level = learndash_plus_get_levels($levelid);
				if(!empty($level) && $level['price'] == '0.00')
				{
					 $now = time();
					$payment = array('amount' => '0.00',
									'currency' => __(' FREE SIGNUP', 'learndash-plus'),
									'timestamp' => $now
									);
					$updated = learndash_plus_update_user_level($user_id, $levelid, $payment);
				}
				$form = "";
			}
			...
			<input type='hidden' name='learndash_plus_user_id' value='".$user_id."'>
			<input type='hidden' name='learndash_plus_membership_id' value='".$button['atts']['itemno']."'>
			<input type='submit' name='learndash_plus_free_signupsubmit' value='" . __('Access Content', 'learndash-plus') . "'>
			</form>";
		}
		else
		{
			$form = "<form method='post' action='".wp_login_url( get_permalink() )."'>
		<input type='hidden' name='learndash_plus_user_id' value='".$user_id."'>
		<input type='hidden' name='learndash_plus_membership_id' value='".$button['atts']['itemno']."'>
		<input type='submit' name='learndash_plus_free_signupsubmit_login' value='" . __('Log-in or Register to View', 'learndash-plus') . "'>
		</form>";
		}
		return $form;
